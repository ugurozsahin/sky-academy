...
describe('the reviewer routine keeps a pulse, and something reads it (#327, #320)', () => {
  ...
  // pulse replace but the stamp checks only the pulse's header line — never the whole body, which a whole-body
  // compare fails on its own success case (#704 review): an append under a still-current header changes the
  // body without changing the header it was appended under, so the header is what a replace must key off, and
  // a replace must keep any such lines already there rather than discard them. This file sits at zero budget
  // headroom and gets trimmed almost every week (the `REVIEWER_PROMPT_BUDGET` history above), which is exactly
  // how a clause like #579's clause (c) or #194's priority order earned a word-for-word pin — this one had
  // none until now.
  it('STEP 1 carries the #460 liveness re-read, word for word and before every unconditional write', () => {
    const p = flat(doc('docs/REVIEWER-PROMPT.md'));
    // Round 2 (PR #704 review): the header check now covers EVERY write but the stamp, including the two
    // branches that fire before this run has ever stamped its own `IN PROGRESS` — the "nothing waiting" cheap
    // exit and STEP 0's `stopped: limit` write. "no other's live" is what lets those two branches replace
    // safely when nothing is live, without a stamp of their own to compare against.
    ...
    // Round 5 (PR #704 review, round 5): the ~90-minute age threshold itself was unsafe — rule 4 sets no depth
    // limit on a review, and #460's own evidence records one spanning close to two hours, so a run genuinely
    // still reviewing past 90 minutes is ordinary, not a bug, and no finite age can tell that apart from a
    // stamp abandoned 90 minutes ago. Age drops out entirely: an `IN PROGRESS:` stamp that is not this run's
    // own is always live, whatever its age, and only ever appended to. A header that is not an `IN PROGRESS:`
    // stamp at all still replaces normally — round 4's shape check, unchanged.
    const LIVENESS = 'Every write but the stamp re-reads first '
      + '(`docs/decisions/009-the-reviewer-pulse-says-who-holds-it.md`): header unchanged from this run\'s '
      + 'stamp, or not an `IN PROGRESS:` stamp at all — replace, keeping `- also reviewed:` lines; another\'s '
      + '`IN PROGRESS:` stamp, whatever its age — append `- also reviewed: <note>`';
    ...
    expect(p, 'the append payload for the cheap-exit case must be spelled out, not left to the run to invent')
      .toContain('`nothing waiting (N open, 0 waiting)`, or `stopped: limit — <what>` if none');
