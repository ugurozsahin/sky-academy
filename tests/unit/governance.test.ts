...
describe('a review that ran ends in a mark, whatever else is true of the branch (#516)', () => {
  ...
  it('clause (c) says finish, and §6 says what finishing is when you may not merge (#579)', () => {
    ...
    expect(step1, 'clause (c) must point at the section that says what it asks for').toContain('§6 has what (c) asks of you');
    const s6 = section6();
  ...
  it('the waiting test carries clause (c) word for word, conjunction included (#579)', () => {
    ...
    // very sentence.
    const CLAUSE_C = 'newer than that block; or (c) it is not a draft, its `review-gate` is green, and open '
      + '\u2014 his marker after a clear moves no commit, so (a) and (b) miss it; \u00a76 has what (c) asks of you '
      + '(#579).';
