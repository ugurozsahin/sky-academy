...
Seven pushes, five hours, 254 lines added to one test file, which ended up modelling the Claude Code skill
loader in regular expressions. Two real defects came out of that growth: a negation word-list that rejected
ordinary English (`only`, `instead`, `no`) in the one line that most needs to be well written, and a
character check asserted against the whole file so that a carriage return anywhere in a skill's body failed
the build.

Across 2026-09-19 there were **18 blocking rounds**, 11 of them on two pull requests. The developer routine
answers at most one stalled block per run and a run takes about 47 minutes, so blocks accumulate faster than
they can be answered — PR #295 reached its fifth round nine hours after it opened.

Every round was defensible on its own. The sum was not, and nothing in the protocol could see the sum. What
ended the loop was not agreement but #161: a second reviewer superseding the sixth block.
...
- **Automate it: have `review-gate` refuse a fourth block.** Rejected. The bar is a judgement, and a gate that
  mechanically refused a block would eventually merge over a real defect — the failure #74 already caused once.
...
- **Cap the size of a review rather than the number of rounds.** Not rejected — deferred. The rounds on #292
  were 7,000–10,000 characters each, which is why answering one took a full run; cost per round is a real
...
- **The cap alone, with no bar.** Rejected: "merge on the fourth round" with no account of what may block is
  how #74 was merged over five open review items.
