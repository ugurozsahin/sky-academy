...
(`GITHUB_TOKEN`/`GH_TOKEN`); only if a snapshot (STEP 5) reports it could not open PRs or comment on
issues does the owner add a `GITHUB_TOKEN` variable (repo scope). If the API is down you cannot write the
heartbeat either: say so in the report.

The **project board** (Projects v2) is **not reachable from a cloud session**: GraphQL-only, and the cloud
proxy answers 403. No run syncs or reads it. The sync (`scripts/board-sync.mjs`) runs on the **owner's Mac**
every 15 minutes and leaves a **pulse** in the open issue `board: heartbeat`, which you read in STEP 1.
...
2026-09-09): neither report it stale nor publish it.
...
The PR title carries `(#<n>)`; the body says `Closes #<n>` when the issue is finished, `Part of #<n>` when you are deferring some of it. GitHub reads a closing keyword next to an issue number wherever it sits in the body, negation included, so check with `node scripts/review-gate.mjs body.md` before you open it — the skill has the full trap and why backticks do not disarm it. Implement with tests (unit for logic, e2e for UI; keep the `window.__sna` hooks working; a *budget* rail only ever goes **down**). Prove it green — `npm test`, `npx tsc --noEmit`, `npm run build && npx playwright test --project=mobile`, desktop too when the change is viewport-sensitive — before you push, not after; the skill has the e2e-scope rule (#96), the browsers-unavailable phrase, and §4's sweep and agent rules (#466). Stop starting new work after ~45 minutes; an unfinished branch is still pushed green with a WIP note, its PR saying `Part of #<n>`, never `Closes #<n>`. Open as a draft, tick "Develop" on the issue, and once the newest CI run on its head is green mark it "Ready for review" (the skill's §5) — that is what puts it in the reviewer routine's next run. **If no CI run appears for that head within a couple of minutes, check `mergeable_state` before waiting longer** (#452): `unknown` means GitHub has not finished computing it — recheck shortly, do not treat it as a green light or a conflict; `dirty` means `main` moved underneath the push and no run will ever start — merge it in the way STEP 2.5 does (`open-pr` §2) and push, which creates the run. Do not review or merge your own PR — the skill has the rest: the governance-PR framing, and one home per rule.
...
pull request closing two issues. They must be reviewable, mergeable and blockable independently, and one going
bad must not hold the other. Say in that pull request what the second item costs in Actions minutes at
the current rate: two items mean two CI runs and the spending limit is deliberately closed.
...
STEP 4 — NOTHING ELIGIBLE? (snapshot: `- query top pick: none eligible`.) Do not invent work no open issue asks for — if it is worth doing, file it with a priority label and the next run picks it up. Instead: QA something merged without review (its thread has no reviewer comment), lower a guard-rail budget you can, thicken unit coverage of `arena`/`visuals`/`tracing` (#43), or write up what you would do next and why. Fix obvious low-risk bugs (wrong ranges, typos, failing tests) in a PR.
