...
1. **No open pull request is waiting for a review this run could do.** `GET /repos/ugurozsahin/sky-academy/pulls?state=open`,
   ...
   newest `REVIEW:` comment is a `REVIEW: CHANGES REQUESTED` **this run may not clear** (one on a pull request this run opened or pushed to); one held **only**
