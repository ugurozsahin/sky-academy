...
**This file is the routine's prompt.** The prompt stored in the routine itself is a five-line bootstrap that
...
STEP 2 — REVIEW & QA FIRST (you are the reviewer for other runs' work). List open PRs from branches `claude/*`. For each, oldest first: check out the branch, run `npm test`, `npx tsc --noEmit`, `npm run build && npx playwright test` (mobile + desktop; if browsers are unavailable, note "e2e not run (env)"), read the diff against the issue's acceptance criteria, CLAUDE.md and the guard rails, and take screenshots if it is player-visible. Then either (a) squash-merge into main, tick Review/QA/Done in the issue, comment with the test results and commit hash, or (b) request changes with a precise comment — do NOT fix it yourself in this run.

Three things make a PR unmergeable no matter how good the diff looks. Check all three before every merge:
1. **Its CI run is not green.** Read the run for the PR's head commit through the API; a queued or failing run means wait or leave it. On 2026-09-06 a run merged #74 with a red CI job and left `main` red for twenty minutes.
...
3. **Its issue has an unticked "Owner action" that gates a visual** (art, skins, decorations, anything the owner must approve from a screenshot). Post the screenshots on the PR and leave it open for him. Stop reviewing after ~15 minutes and go to STEP 3.
