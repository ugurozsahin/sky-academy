...
- **add or remove `blocked`** on more than three.

Each is counted on its own — four separate totals, not one sum, so six grants notify him even in a run that
...
`blocked` sits lower than the rest, at three, because it is the only one of the four that moves work **out**
